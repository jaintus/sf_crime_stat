...
def udf_convert_time(timestamp):
    ...
    '''
    return datetime.strptime(str(timestamp), '%Y-%m-%dT%H:%M:%S.000').strftime('%Y%m%y%H')
...
def run_spark_job(spark):
    ...
    calls_per_2_days = distinct_table.withWatermark(
            "call_date_time", "2 days"
        ).groupBy(
            psf.window(psf.col("call_date_time"), "2 days")
        ).count()

    # query_df = counts_df
    # query_df = calls_per_2_days 

    for query_df in [counts_df, calls_per_2_days ]:
        # write output stream
        query = query_df \
                .writeStream \
                .outputMode('complete') \
